    ...
    <aside className={`${mobile ? "max-h-[85vh]" : "h-[calc(100vh-56px)] lg:sticky lg:top-[56px]"} sidebar-scroll flex flex-col border-r border-[var(--border)] bg-[var(--bg-secondary)]`}>
      <div className="flex-1 overflow-y-auto p-5 pb-32">
        ...
        <AccordionItem value="tax" className="rounded-xl border border-[var(--border)] bg-[var(--bg-card)] px-5 py-4">
          ...
          <AccordionContent className="grid gap-4">
            ...
            <SliderInput
              label="Number of dependents (for child tax credit)"
              value={inputs.numDependents}
              min={0} max={5} step={1}
              onChange={(v) => setInput('numDependents', v)}
            />
            
            <SliderInput
              label="State income tax rate"
              value={inputs.stateIncomeTaxRate}
              min={0} max={0.133} step={0.001}
              format="percent"
              onChange={(v) => setInput('stateIncomeTaxRate', v)}
            />
            
            <SliderInput
              label="Pre-tax savings rate (401k, HSA, etc.)"
              value={inputs.preTaxSavingsRate}
              min={0} max={0.30} step={0.01}
              format="percent"
              onChange={(v) => setInput('preTaxSavingsRate', v)}
            />
            
            <SliderInput
              label="After-tax savings rate (of take-home pay)"
              value={inputs.afterTaxSavingsRate}
              min={0} max={0.60} step={0.01}
              format="percent"
              onChange={(v) => setInput('afterTaxSavingsRate', v)}
            />
                ...
                return (
                  <div className="grid gap-1 text-xs">
                    <div className="flex justify-between">
                      <span className="text-[var(--text-muted)]">Combined gross income</span>
                      <span className="font-bold">{formatCompactCurrency(combined)}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-[var(--text-muted)]">Est. total tax burden</span>
                      <span className="font-bold text-[var(--danger)]">
                        -{formatCompactCurrency(totalTax)} 
                        ({(combined > 0 ? (totalTax/combined*100) : 0).toFixed(0)}%)
                      </span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-[var(--text-muted)]">After-tax take-home</span>
                      <span className="font-bold">{formatCompactCurrency(afterTax)}</span>
                    </div>
          ...
          <AccordionItem value="events" className="rounded-xl border border-[var(--border)] bg-[var(--bg-card)] px-5 py-4">
            ...
            <AccordionContent className="grid gap-d text-[var(--text-primary)] mb-1">
                  What are lumpy events?
                </p>
                <p className="text-[11px] text-[var(--text-muted)] leading-relaxed">
                  One-time cash events that don't follow a regular pattern — 
                  like a carried interest distribution, real estate sale, 
                  inheritance, or large capital call. Enter the amount you 
                  expect to receive (positive) or pay (negative), and the 
                  age you expect it to happen. Confidence affects how often 
                  this event fires across simulations.
                </p>
              </div>

              {/* Quick-add buttons with better labels */}
              <div>
                ...
                <div className="grid grid-cols-2 gap-2">
                  {[
                    { 
                      ...
                      sublabel: 'Income, LTCG taxed',
                      type: 'carry' 
                    }
                     label: '
                    { 
                      label: '📈 Annual Bonus', 
                      sublabel: 'Income, ordinary taxed',
                      type: 'bonus' 
                    },
                    { 
                      label: '💸 Capital Call', 
                      sublabel: 'Outflow, no tax',
                      type: 'call' 
                    },
                  ].map(btn => (
                    <button
                      ...
                      onClick={() => {
                        const defaults = 
                          btn.type === 'carry' 
                            ? { label: "Carry Distribution", taxType: "ltcg" as const, 
                                amount: 1500000, confidence: "medium" as const, 
                                btn.type === 're'
                            ? { label: "Real Estate Sale", taxType: "ltcg" as const, 
                                amount: 800000, confidence: "medium" as const, 
                                probon'
                         probability: 1.0 };
                        addLumpyEvent({
              ...
              .map((event) => (
                <div key={event.id} className="rounded-xl border border-[var(--border)] bg-[var(--bg-card)] p-4 grid gap-3">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <span className={cn(
                        ...
                        event.taxType === 'ltcg' 
                          ...
                          : event.taxType === 'label className="text-[10px] font-bold uppercase tracking-widest text-[var(--text-muted)]">
                  ...
                  {/* Age + Amount side by side */}
                  <div className="grid grid-cols-2 gap-3">
                    <div className="grid gap-110px] font-bold uppercase tracking-widest text-[var(--text-muted)]">
                        Your Age When It Happens
                      <ue ndar year ~{new Date().getFullYear() + (event.year - inputs.currentAge)}
                      </p>
                    </div>
                    <div className="grid gap-1">
                      <label className="text-[10px] font-bold uppercase tracking-widest text-[var(--text-muted)]">
                        Amount {event.amount >= 0 ? "(receive)" : "(pay out)"}
                      </label>
                      <div className="relative">
                        <span className={cn(
                          "absolute left-2 top-1/2 -translate-y-1/2 text-xs font-bold",
                        ...
                        <Input
                          ...
                          className={cn(
                            "h-8 pl-7 text-xs font-semibold",
                            event.amount >= 0 ? "text-[var(--success)]" : "text-[var(--danger)]"
                          )}
                        />
                      </div>
                      <button
                        onClick={() => updateLumpyEvent({
                          ...event, amount: -event.amount
                        })}
                        className="text-[9px] text-[var(--text-muted)] hover:text-[var(--accent)] text-left transition-colors"
                      >
                        Switch to {event.amount >= 0 ? "outflow" : "inflow"} ↕
                      </button>
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div className="grid gap-1">
                      ...
                      <div className="flex gap-1">
                        {([
                          { key: 'low', label: 'Low', pct: '40%', prob: 0.4 },
                          { key: 'medium', label: 'Med', pct: '70%', prob: 0.7 },
                          { key: 'high', label: 'High', pct: '90%', prob: 0.9 },
                        ] as const).map(c => (
                          <button
                         
                            title={`${c.pct} chance of occurring in any simulation`}
                            className={cn(
                              "flex-1 rounded py-1 text-[9px] font-bold transition-colors",
                              event.confidence === c.key
                                ? "bg-[var(--accent)] text-white"
                                : "bg-white/10 text-[var(--text-muted)]"
                            )}
                          >
                            {c.label}
                            <span className="block text-[8px] opacity-70">{c.pct}</span>
                          </button>
                        ))}
                      </div>
                      <p className="text-[9px] text-[var(--text-muted)]">
                        % chance this fires in each simulation run
                      </p>
                    </div>
                    <div className="grid gap-1">
                      <label className="text-[10px] font-bold uppercase tracking-widest text-[var(--text-muted)]">
                        Tax Treatment
                        value={event.taxType}
                        onChange={(e) => updateLumpyEvent({ed-md border border-[var(--border)] bg-[var(--bg-primary)] px-2 text-[10px] text-[var(--text-primary)]"
             >            <option value="none">No tax (capital return)</option>
                        <option value="ltcg">Long-term capital gains (~24%)</option>
                        <option value="ordinary">Ordinary income (~37%+)</option>
                      </select>
                      <p className="text-[9px] text-[var(--text-muted)]">
                        Net after-tax: {formatCompactCurrency(
                          event.taxType === 'ltcg' 
                            ? event.amount * 0.76
                            : event.taxType === 'ordinary'
                              ? event.amount * 0.60
                              : event.amount
                        )}
                ...
                className="w-full rounded-xl border border-dashed border-[var(--border)] py-3 text-xs font-semibold text-[var(--text-muted)] hover:border-[var(--accent)] hover:text-[var(--accent)] transition-colors"
              >
                + Add Custom Event
              </button>
            </AccordionContent>
                ...
                <div key={event.id} className="rounded-lg border border-[var(--border)] bg-white/[0.03] p-3 grid gap-3">
                  ...
                  <div className="grid grid-cols-2 gap-2 text-xs">
                    <label className="grid gap-1">
                      <span className="text-[var(--text-muted)] uppercase tracking-widest text-[10px]">Your age at start</span>e
                      dder-[var(--border)] bg-transparent px-2 text-[var(--text-primary)]" />
                    </label>
                    <label className="grid gap-1">
                      <span className="text-[var(--text-muted)] uppercase tracking-widest text-[10px]">Annual cost</span>
                      <lN1u{t updated = inputs.collegeEvents.map(ev => ev.id === event.id ? {...ev, years: Number(e.target.value)} : ev);
      ...
      <div className="sticky bottom-0 border-t border-[var(--border)] bg-[var(--bg-secondary)] p-4">
        <div className="grid grid-cols-2 gap-x-4 gap-y-2">
          <div className="border-re="text-[10px] font-bold uppercase text-[var(--text-muted)]">Net Worth</p>
